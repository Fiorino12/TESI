...
class rBergomi(object):
    ...
    def Y(self, dW):
        ...
        for k in np.arange(2, 1 + self.s, 1):
            G[k] = g(b(k, self.H - 0.5)/self.n, self.H - 0.5)
    ...
    def V(self, Y, xi = 1.0, nu = 1.0):
        ...
        H = self.H
        t = self.t
        V = xi * np.exp(nu * Y - 0.5 * nu**2 * t**(2 * H))
        return V
